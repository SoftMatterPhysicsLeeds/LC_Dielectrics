...
class lcd_ui():
    ...
    def _make_control_window(self):
        with dpg.window(label = "Status", pos = [0,0], width = 600, no_resize=True, no_collapse= True, no_close = True, no_move = True) as self.control_window: 
            ...
            with dpg.window(label="Measurement Settings",pos= [0, 100]  ,width = 600, no_resize=True, no_collapse= True, no_close = True, no_move = True):
                with dpg.group(horizontal=True):
            ...
            with dpg.window(label = "Frequency List", width = 300, pos = [0, 200], no_resize=True, no_collapse= True, no_close = True, no_move = True):
                self.freq_list = variable_list(*make_variable_list_frame(20.0))
            
            with dpg.window(label = "Voltage List", width = 300, pos = [300, 200], no_resize=True, no_collapse= True, no_close = True, no_move = True):
                self.volt_list = variable_list(*make_variable_list_frame(1.0))

            with dpg.window(label = "Temperature List", width = 600, pos = [0, 323], no_resize=True, no_collapse= True, no_close = True, no_move = True):
                with dpg.group(horizontal=True):
                    self.temperature_list =  variable_list(*make_variable_list_frame(25.0))
                    with dpg.group():
                        with dpg.group(horizontal=True):
