...
class lcd_ui:
    ...
    def _make_control_window(self):
        with dpg.window(
            ...
            with dpg.window(pos=[0, VIEWPORT_HEIGHT/6 * 5], no_title_bar=True, width=VIEWPORT_WIDTH/2, height = VIEWPORT_HEIGHT/6):
                with dpg.group(horizontal=True):
                    self.start_button = dpg.add_button(
                        label="Start", pos=[10, VIEWPORT_HEIGHT/6 * 0.13 ], width=285, height=50
                    )
                    self.stop_button = dpg.add_button(
                        label="Stop", pos=[VIEWPORT_WIDTH/4 + 10, VIEWPORT_HEIGHT/6 * 0.13], width=285, height=50
                    )
